  ...
  try {
    ...
    if (!name || !email || !subject || !message || !rating) {
      return res.status(400).json({
        message:
          "All fields are required (name, email, subject, message, rating)",
      });
    }
    ...
    try {
      ...
      if (notifications.length > 0) {
        ...
        console.log(
          `Created ${notifications.length} notifications for new feedback from ${feedback.name}`,
        );
      }
    } catch (notificationError) {
      // Log error but don't fail the feedback submission
      console.error(
        "Error creating feedback notifications:",
        notificationError,
      );
    }
...
router.get("/feedback", auth, async (req, res) => {
  try {
    ...
    // Parse pagination; if limit is absent or "all", return every feedback item.
    const parsedPage = Math.max(parseInt(page, 10) || 1, 1);
    const normalizedLimit =
      typeof limit === "string" && limit.trim().toLowerCase() === "all"
        ? NaN
        : parseInt(limit, 10);
    const hasPagination =
    ...
    const feedbackQuery = Feedback.find(filter)
      ...
      .lean();
    if (hasPagination) {
      const skip = (parsedPage - 1) * normalizedLimit;
      feedbackQuery.skip(skip).limit(normalizedLimit);
    }

    const feedbackDocs = await feedbackQuery;
    const feedback = feedbackDocs.map(normalizeFeedbackRecord);

    const responseLimit = hasPagination
      ? normalizedLimit
    ...
    res.json({
      feedback,
      pagination: {
        current: hasPagination ? parsedPage : 1,
        pages,
        total,
        limit: responseLimit,
      },
...
router.get("/feedback/:id", auth, async (req, res) => {
  try {
    ...
    const feedbackRecord = await Feedback.findById(req.params.id)
      .select("-__v")
      .lean();
    ...
    const feedback = normalizeFeedbackRecord(feedbackRecord);
    res.json(feedback);
  } catch (error) {
    console.error("Error fetching feedback:", error);
    ...
    const stats = await Feedback.aggregate([
      {
        $group: {
          ...
          pendingCount: {
            $sum: {
              $cond: [
                { $eq: [{ $ifNull: ["$status", "pending"] }, "pending"] },
                1,
                0,
              ],
            },
          ...
          reviewedCount: {
            $sum: {
              $cond: [
                { $eq: [{ $ifNull: ["$status", "pending"] }, "reviewed"] },
                1,
                0,
              ],
            },
          ...
          resolvedCount: {
            $sum: {
              $cond: [
                { $eq: [{ $ifNull: ["$status", "pending"] }, "resolved"] },
                1,
                0,
              ],
            },
